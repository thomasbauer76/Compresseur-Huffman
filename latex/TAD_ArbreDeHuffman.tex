...
\begin{tad}
  ...
  \tadDependances{\textbf{Statistiques}, \textbf{Octet}, \naturel, \booleen}
  \begin{tadOperations}{obtenirFilsGauche}
  
    \tadOperation{arbreDeHuffman}{\textbf{Statistiques}}{ArbreDeHuffman}
  ...
  \parbox{\linewidth}{\raggedright
      \begin{tadAxiomes}
            \tadAxiome{estUneFeuille(arbreDeHuffman(s))}
            \tadAxiome{non(estUneFeuille(ajouterRacine(a_g,a_d)))}
            \tadAxiome{obtenirOctet(arbreDeHuffman(s)) = Statistiques.obtenirOctet(s)}
            \tadAxiome{obtenirFrequence(arbreDeHuffman(s)) = Statistiques.obtenirFrequence(s)}
